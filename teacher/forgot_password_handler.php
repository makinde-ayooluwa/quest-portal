...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $user_type = 'teacher'; // Fixed for teacher handler
    ...
    $stmt = $conn->prepare("SELECT id FROM staffs WHERE email = ? AND staff_role = 'Teacher'");
    $stmt->bind_param("s", $email);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows === 0) {
        $_SESSION['error'] = "No teacher account found with this email address.";
        $stmt->close();
        $conn->close();
        header('Location: forgot_password.php');
        exit();
    ...
    if ($insert_stmt->execute()) {
        ...
        try {
            ...
            // Content
            $reset_link = "http://localhost/quest-portal/teacher/reset_password.php?token=" . $token . "&type=" . $user_type;
            $mail->isHTML(true);
            $mail->Subject = 'Password Reset Request - Quest Portal Teacher';
            $mail->Body = "
                <h2>Password Reset Request</h2>
                <p>You have requested to reset your teacher password for Quest Portal.</p>
