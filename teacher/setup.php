...
elseif (/*$teacher->getSpecificTeacher($pdo, $_GET["id"]) && */$teacher->getSpecificTeacher($pdo, $_GET["portal_code"])["portal_code"] != $_GET["portal_code"]) {
?>
Page blocked
<?php
} elseif ($teacher->getSpecificTeacher($pdo, $_GET["portal_code"])["staff_role"] !== "Teacher") {
?>
Page blocked
<?php
} elseif ($teacher->getSpecificTeacher($pdo, $_GET["portal_code"])["account_verification"] == "Verified") {
?>
Teacher is verified
<?php
} else {
    $data = $teacher->getSpecificTeacher($pdo, $_GET["portal_code"]);
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Account Setup - Quest Teacher</title>
    <style>
    .form-container {
        border-radius: 12px;
        width: 442px;
        ...
        <form action="setup_handler.php" method="post" enctype="multipart/form-data">
            ...
            <div class="my-2">
                <div class="mb-2">
                    ...
                    <input type="text" id="Pportal_code" name="portal_code" readonly
                        value="<?php echo $_GET["portal_code"] ?>" class="form-control">
                </div>
                <div class="mb-2">
                    <label for="Eemail" class="form-label">Email</label>
                    <input type="email" id="Eemail" name="email" readonly class="form-control"
